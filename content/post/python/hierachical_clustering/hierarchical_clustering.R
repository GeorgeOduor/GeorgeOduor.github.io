...
- __Retail/Marketing__
    
    - _Identifying buying patterns of customers._
    - _Recommending new books or movies to new customers_
 
- __Banking:__

    - _Fraud detection in credit card use_
    - _Identifying clusters of customers_
 
- __Insurance:__

    - _Fraud detection in claims analysis._
    - _Insurance risk of customers._

- __Publication:__

    - _Auto-categorizing news based on their content_
    - _Recommending similar news articles_
 
- __Medicine:__
  - _Characterizing patient behavior_
  
- __Biology:__
  - _Clustering genetic markers to identify family ties._
...
1. __Divisive (Top-down approach)__
Start with all observations and break it down into smaller pieces.

2. __Agglomerative(Bottom-up approach)__
Each observation starts the cluster and pairs up as they move the hierarchy. Agglomeration simply means to collect things up. The agglomerative approach is very popular and works in a similar way which humans think.

## __Understanding a dendrogram and the agglomerative approach.__

_So how does it work?_

This method builds a hierarchy from individual elements by progressively merging clusters.

In a dendrogram, each leaf represents one of the data points. As we move up the tree some observations fuse together to form branches.

These observations that fuse together are very similar to each other or close to one another. Further up the tree, branches fuse with other branches or observations.
...
Let's understand how the program makes the dendrogram in the right figure. Assume the points represent cities in Kenya and we want to cluster them based on their distance from each other. Distance between the points can be calculated by many methods including euclidian distance.

Looking at the scatter plot on the left side points 3 and 4 are the closest therefore they will be fused together and they will form a cluster. This leaves us with n -1 =7 clusters. The distance between them is represented by the height of the cluster formed.

By checking the distance between points 1 and 2 relative to other points we find they are the closest so we will make a cluster with them. Using the first cluster between 3 and 4 we can see that it's closer to five than the cluster between 1 and 2 so so we will fuse this cluster with point 5.
...
This problem is easily solved by the concept of the linkage which defines the dissimilarity between two groups of observations. There are 4 main types of linkages namely:

1. __Complete linkage:__ takes the largest of the dissimilarities between the observations in two clusters after computing all pairwise dissimilarities.
1. __Average linkage__ takes the average of the dissimilarities between the observations in two clusters after computing all pairwise dissimilarities.
1. __Single linkage__ takes the smallest of the dissimilarities between the observations in two clusters after computing all pairwise dissimilarities.
1. __Centroid linkage__: takes the dissimilarity between the centroid of cluster A and centroid of cluster B
